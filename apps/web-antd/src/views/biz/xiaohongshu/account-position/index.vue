...
import { XiaohongshuAccountPositionApi } from '#/api/xiaohongshu/account-position';
...
const placeholderText = computed(
  () =>
    '请描述您的特长、兴趣爱好、目标受众等信息，AI将为您生成精准的账号定位建议...',
);
...
async function handleGenerate() {
  ...
  try {
    ...
    const data: XiaohongshuAccountPositionApi.AccountPositionGenerateReqVO = {
      userMessage: userMessage.value,
    };

    const response =
      await XiaohongshuAccountPositionApi.generateAccountPosition(data);
    generatedResult.value = response;
    message.success('账号定位生成成功！');
  } catch (error) {
    console.error('生成账号定位失败:', error);
    message.error('账号定位生成失败，请稍后重试');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <Page
    :title="$t('小红书账号定位')"
    description="🎯 AI帮您找准账号定位，明确发展方向，打造独特个人品牌！"
    auto-content-height
  >
    <div class="space-y-6">
      <!-- 主题输入区域 -->
          ...
          <template #message>
            <div class="text-sm">
              ...
              <div class="mt-1 text-gray-600">
                ...
                • AI 会根据您的描述生成精准的账号定位建议<br />
                • 建议详细描述您的特长、兴趣、资源和目标<br />
                •
                如果您有优秀的账号定位案例，欢迎联系我们添加到模板库中(微信:sunnao678)
              </div>
            </div>
          </template>
        </Alert>
