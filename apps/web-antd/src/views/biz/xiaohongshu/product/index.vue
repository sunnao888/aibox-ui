...
import { XiaohongshuProductApi } from '#/api/xiaohongshu/product';
...
const placeholderText = computed(
  () =>
    '请描述您要推荐的产品信息，包括产品特点、功效、使用体验等，AI将为您生成种草文案...',
);
...
async function handleGenerate() {
  ...
  try {
    ...
    const data: XiaohongshuProductApi.ProductGenerateReqVO = {
      userMessage: userMessage.value,
    };

    const response = await XiaohongshuProductApi.generateProduct(data);
    generatedResult.value = response;
    message.success('种草文案生成成功！');
  } catch (error) {
    console.error('生成种草文案失败:', error);
    message.error('种草文案生成失败，请稍后重试');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <Page
    :title="$t('小红书种草文案')"
    description="🌱 AI帮您创作有说服力的种草文案，让您的产品推荐更打动人心！"
    auto-content-height
  >
    <div class="space-y-6">
      <!-- 主题输入区域 -->
          ...
          <template #message>
            <div class="text-sm">
              ...
              <div class="mt-1 text-gray-600">
                ...
                • AI 会根据您的描述生成有说服力的种草文案<br />
                • 建议详细描述产品特点、功效、使用体验等信息<br />
                •
                如果您有优秀的种草文案案例，欢迎联系我们添加到模板库中(微信:sunnao678)
              </div>
            </div>
          </template>
        </Alert>
