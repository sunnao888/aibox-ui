...
import { XiaohongshuTopicApi } from '#/api/xiaohongshu/topic';
...
const placeholderText = computed(
  () =>
    '请描述您的兴趣爱好、专长领域或目标受众，AI将为您生成适合的小红书内容选题...',
);
...
async function handleGenerate() {
  ...
  try {
    ...
    const data: XiaohongshuTopicApi.TopicGenerateReqVO = {
      userMessage: userMessage.value,
    };

    const response = await XiaohongshuTopicApi.generateTopic(data);
    generatedResult.value = response;
    message.success('选题灵感生成成功！');
  } catch (error) {
    console.error('生成选题灵感失败:', error);
    message.error('选题灵感生成失败，请稍后重试');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <Page
    :title="$t('小红书选题灵感')"
    description="💡 AI帮您发现热门话题，激发创作灵感，让您的小红书内容更受欢迎！"
    auto-content-height
  >
    <div class="space-y-6">
      <!-- 主题输入区域 -->
          ...
          <template #message>
            <div class="text-sm">
              ...
              <div class="mt-1 text-gray-600">
                ...
                • AI 会根据您的描述生成适合的小红书选题方向<br />
                • 建议详细描述您的兴趣、专长或目标受众群体<br />
                •
                如果您有优秀的选题案例，欢迎联系我们添加到模板库中(微信:sunnao678)
              </div>
            </div>
          </template>
        </Alert>
