...
import { XiaohongshuTitleApi } from '#/api/xiaohongshu/title';
...
const placeholderText = computed(
  () => '请描述您的内容主题或关键词，AI将为您生成多个吸引眼球的爆款标题...',
);
...
async function handleGenerate() {
  ...
  try {
    ...
    const data: XiaohongshuTitleApi.TitleGenerateReqVO = {
      userMessage: userMessage.value,
    };

    const response = await XiaohongshuTitleApi.generateTitle(data);
    generatedResult.value = response;
    message.success('爆款标题生成成功！');
  } catch (error) {
    console.error('生成爆款标题失败:', error);
    message.error('爆款标题生成失败，请稍后重试');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <Page
    :title="$t('小红书爆款标题')"
    description="🔥 AI帮您创作吸引眼球的爆款标题，让您的内容脱颖而出！"
    auto-content-height
  >
    <div class="space-y-6">
      <!-- 主题输入区域 -->
          ...
          <template #message>
            <div class="text-sm">
              ...
              <div class="mt-1 text-gray-600">
                ...
                • AI 会根据您的描述生成多个吸引眼球的标题选择<br />
                • 建议提供内容主题、关键词或目标受众信息<br />
                •
                如果您有优秀的爆款标题案例，欢迎联系我们添加到模板库中(微信:sunnao678)
              </div>
            </div>
          </template>
        </Alert>
