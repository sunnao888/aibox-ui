...
import { XiaohongshuNoteApi } from '#/api/xiaohongshu/note';
...
const placeholderText = computed(
  () =>
    '请描述您要分享的内容主题、经历或产品，AI将为您创作吸引人的小红书笔记...',
);
...
async function handleGenerate() {
  ...
  try {
    ...
    const data: XiaohongshuNoteApi.NoteGenerateReqVO = {
      userMessage: userMessage.value,
    };

    const response = await XiaohongshuNoteApi.generateNote(data);
    generatedResult.value = response;
    message.success('笔记生成成功！');
  } catch (error) {
    console.error('生成笔记失败:', error);
    message.error('笔记生成失败，请稍后重试');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <Page
    :title="$t('小红书笔记助手')"
    description="📝 AI帮您创作引人入胜的小红书笔记，提升内容质量和用户互动！"
    auto-content-height
  >
    <div class="space-y-6">
      <!-- 主题输入区域 -->
      <Card>
        <div class="space-y-6">
          ...
          <!-- 主题输入 -->
          <div>
            <div class="mb-2 text-sm font-medium text-gray-700">内容描述</div>
            <Input.TextArea
          ...
          <template #message>
            <div class="text-sm">
              ...
              <div class="mt-1 text-gray-600">
                ...
                • AI 会生成符合小红书风格的优质笔记内容<br />
                • 建议详细描述您的主题、目标受众和想要传达的信息<br />
                •
