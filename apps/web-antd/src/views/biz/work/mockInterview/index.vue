...
import { MockInterviewApi } from '#/api/work/mockInterview';
...
const placeholderText = computed(
  () =>
    '请详细描述您的面试信息，包括应聘职位、公司背景、技能要求、面试类型（技术面试/行为面试/综合面试）等，系统将为您生成专业的模拟面试题目和场景...',
);
...
async function handleGenerate() {
  ...
  try {
    ...
    const data: MockInterviewApi.MockInterviewGenerateReqVO = {
      userMessage: userMessage.value,
    };

    const response = await MockInterviewApi.generateMockInterview(data);
    generatedResult.value = response;
    message.success('模拟面试生成成功！');
  } catch (error) {
    console.error('生成模拟面试失败:', error);
    message.error('模拟面试生成失败，请稍后重试');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <Page
    :title="$t('AI 模拟面试助手')"
    description="🎯 使用 AI 为您生成专业的模拟面试题目和场景，帮助您充分准备面试。"
    auto-content-height
  >
    <div class="space-y-6">
      <!-- 面试信息输入区域 -->
          ...
          <template #message>
            <div class="text-sm">
              ...
              <div class="mt-1 text-gray-600">
                • 点击下方模板可快速填入示例内容，帮助您更好地描述面试信息<br />
                • AI 会根据您的描述生成专业的面试题目和模拟场景<br />
                • 建议详细描述职位要求、技能需求和面试类型<br />
                •
                如果您有优秀的面试模板，欢迎联系我们添加到模板库中(微信:sunnao678)
              </div>
            </div>
          </template>
        </Alert>
