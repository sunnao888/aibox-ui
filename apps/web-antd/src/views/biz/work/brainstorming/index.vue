...
import { WorkBrainstormingApi } from '#/api/work/brainstorming';
...
const placeholderText = computed(
  () =>
    '请详细描述您需要头脑风暴的主题或问题，系统将为您生成创意想法和解决方案...',
);
...
async function handleGenerate() {
  ...
  try {
    ...
    const data: WorkBrainstormingApi.BrainstormingGenerateReqVO = {
      userMessage: userMessage.value,
    };

    const response = await WorkBrainstormingApi.generateBrainstorming(data);
    generatedResult.value = response;
    message.success('头脑风暴生成成功！');
  } catch (error) {
    console.error('生成头脑风暴失败:', error);
    message.error('头脑风暴生成失败，请稍后重试');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <Page
    :title="$t('AI 头脑风暴生成器')"
    description="🧠 使用 AI 激发您的创意思维，为您的问题和挑战提供多角度的创新解决方案和灵感。"
    auto-content-height
  >
    <div class="space-y-6">
      <!-- 主题输入区域 -->
      <Card>
        <div class="space-y-6">
          ...
          <!-- 主题输入 -->
          <div>
            <div class="mb-2 text-sm font-medium text-gray-700">主题描述</div>
            <Input.TextArea
          ...
          <template #message>
            <div class="text-sm">
              ...
              <div class="mt-1 text-gray-600">
                •
                点击下方模板可快速填入示例内容，帮助您更好地描述头脑风暴主题<br />
                • AI 会根据您的描述生成多角度的创意想法和解决方案<br />
                • 建议详细描述您的问题背景和期望的创新方向<br />
                •
                如果您有优秀的头脑风暴模板，欢迎联系我们添加到模板库中(微信:sunnao678)
              </div>
            </div>
          </template>
        </Alert>
